...
def Lasso_Frank(x, y, pol_degree, noise=0, plot_index=-1):
    ...
    for i in range(nlambdas):
        lmb = lambdas[i]
        Lassobeta = np.linalg.inv(X_train.T @ X_train+lmb*I) @ X_train.T @ y_train
    #    print(Lassobeta)
        # and then make the prediction
        ytilde_train[:,i] = X_train @ Lassobeta
        ytilde_test[:,i] = X_test @ Lassobeta
